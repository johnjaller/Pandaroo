      ...
      stripe.checkout.sessions.listLineItems(
        ...
        async function (err, lineItems) {
          ...
          for (let i = 0; i < lineItems.data.length; i++) {
            ...
            let menuId = await classInstance.checkoutService.getMenuId(
              lineItems.data[i].description
            );
            menuId = menuId[0].id;
            console.log(menuId);
            products.push({
          ...
          classInstance.checkoutService
            ...
            .then(async (deliveryId) => {
              for (let i = 0; i < products.length; i++) {
                await classInstance.checkoutService.insertOrderDetail(
                  deliveryId[0],
                  products[i].menu_id,
                  products[i].quantity
                );

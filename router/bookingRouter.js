...
class BookingRouter{
    ...
    let bookmark=await this.bookingService.getUserBookmarkStatus(req.user.id,req.params.restId)
    let bookmarkClass
  ...
  let dish=await this.bookingService.getRestDishes(req.params.restId,"soup&salad")
    console.log(dish)
  ...
  bookmarkClass='fas'
    ...
    let bookmark=await this.bookingService.getUserBookmarkStatus(req.user.id,req.params.restId)
    let bookmarkClass
    let rating=await this.bookingService.getRestRating('rest_id',req.params.restId)
    if(rating.length!=0)
    {
    rating=Math.round(rating.map(item=>Number(item.rating)).reduce((a,b)=>a+b)/rating.length*2)/2
    console.log(rating)
    ...
    restDetail[0]['rating']=rating
  let dish=await this.bookingService.getRestDishes(req.params.restId,req.params.category)
    console.log(dish)
    console.log(bookmark)
    if(bookmark.length!=0)
    {
  bookmarkClass='fas'
    ...
    return res.render('userBooking',{layout:'user',restaurant:restDetail[0],dish:dishItems,bookmark:bookmarkClass
      ...
      postBooking(req,res)
      {
        ...
        return this.bookingService.insertBooking(req.params.restId,req.user.id,req.body.bookingTime,req.body.bookingDate,req.body.specialRequest,req.body.noOfGuest,'confirmed').then(async(bookingId)=>{
            for(let i=0;i<bookingCart.item.length;i++)
            {
             await this.bookingService.insertBookingDetail(bookingId[0],bookingCart.item[i].menuId,bookingCart.item[i].amount)
            }
        })
      }
      deleteBooking(req,res)
      {
          return this.bookingService.deleteBookingDetail(req.params.bookingId).then(()=>{

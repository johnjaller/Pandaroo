...
class OrderRouter{
    ...
    route() {
        ...
        router.delete("/:orderId", this.deleteUserOrder.bind(this));
        return router;
      }
      async getRestOrderDefault(req,res)
      {
          let deliverySwitch=await this.orderService.getDeliveryStatus(req.params.restId)
          if(deliverySwitch[0].delivery===true)
            ...
            console.log(rating)
              ...
              let restDetail=await this.orderService.getRestDetail(req.params.restId)
              restDetail[0]['rating']=rating.toString()
              let bookmark=await this.orderService.getUserBookmarkStatus(req.user.id,req.params.restId)
              let bookmarkClass
              let dish=await this.orderService.getRestDishes(req.params.restId,'soup&salad')
      ...
      async getRestOrderCategory(req,res)
      {
          ...
          if(deliverySwitch[0].delivery===true)
            ...
            console.log(rating)
              ...
              let bookmark=await this.orderService.getUserBookmarkStatus(req.user.id,req.params.restId)
              let bookmarkClass
              let dish=await this.orderService.getRestDishes(req.params.restId,req.params.category)
              if(bookmark.length!=0)
                {
                bookmarkClass='fas'
                }else{
      ...
      {
          return this.orderService.deleteOrderDetail(req.params.orderId).then(()=>{
              this.orderService.deleteDeliveryOrder(req.params.orderId).then(()=>{
                  res.send('success')
              })
          })
      }

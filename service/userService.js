class UserService {
  ...
  async getUserBooking(userId) {
    ...
    for (let i = 0; i < booking.length; i++) {
      ...
      let bookingDetail = await this.knex("booking_detail")
        ...
        .select("menu.item", "booking_detail.quantity", "menu.price")
  ...
  async getUserOrder(userId) {
    ...
    for (let i = 0; i < delivery.length; i++) {
      ...
      let orderDetail = await this.knex("order_detail")
        .join("delivery", "delivery_id", "delivery.id")
        .join("menu", "menu_id", "menu.id")
        .select("menu.item", "order_detail.quantity", "menu.price")
  ...
  async getUserBookmark(userId) {
    ...
    for (let i = 0; i < bookmark.length; i++) {
      ...
      let tagName = await this.knex("tag_rest_join")
        .join("restaurant", "tag_rest_join.rest_id", "restaurant.id")

class BookingService{
    ...
    insertBooking(restId,userId,bookingTime,bookingDate,specialRequest,noOfGuest,bookingStatus)
    {
     return this.knex('booking').insert({rest_id:restId,account_id:userId,booking_time:bookingTime,booking_date:bookingDate,special_request:specialRequest,no_of_ppl:noOfGuest,booking_status:bookingStatus}).returning('id')
    }
    insertBookingDetail(bookingId,menuId,amount)
    {
     return this.knex('booking_detail').insert({booking_id:bookingId,menu_id:menuId,quantity:amount})
    }
